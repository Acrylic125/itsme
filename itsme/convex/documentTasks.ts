...
function getStructuralChildBlockIds(
  data: (typeof blockDataValidator)["type"]
): Id<"blocks">[] {
  ...
  return data.children.map((c) => c.blockId);
}

type StructuralChildRef = {
  childId: Id<"blocks">;
  span?: number;
};

function getStructuralChildRefs(
  data: (typeof blockDataValidator)["type"]
): StructuralChildRef[] {
  if (data.type === "text") return [];
  if (data.type === "section" || data.type === "list") {
    return data.children.map((childId) => ({ childId }));
  }
...
function insertStructuralChildIntoData(args: {
  data: (typeof blockDataValidator)["type"];
  insertIndex: number;
  childId: Id<"blocks">;
  span?: number;
  ...
  const idx = Math.max(
    0,
    Math.min(args.insertIndex, getStructuralChildRefs(args.data).length)
  );

  if (args.data.type === "text") {
    throw new Error("Cannot insert a structural child into a text block.");
  }
  if (args.data.type === "section") {
    return {
      type: "section",
      children: [
        ...args.data.children.slice(0, idx),
        args.childId,
        ...args.data.children.slice(idx),
      ],
...
function getOptionalDataRef(
  data: (typeof blockDataValidator)["type"]
): Id<"blocks"> | undefined {
  return data.ref;
}

async function masterAnchorRefForLayoutRow(
  ctx: MutationCtx,
  blockRow: Doc<"blocks">,
  masterDocumentId: Id<"documents">
): Promise<Id<"blocks"> | null> {
  const ref = getOptionalDataRef(blockRow.data);
  if (!ref) return null;
  const target = await ctx.db.get(ref);
  if (!target || target.documentId !== masterDocumentId) return null;
  return ref;
}
...
function cloneBlockData(
  ...
  return JSON.parse(JSON.stringify(data)) as (typeof blockDataValidator)["type"];
}

/**
 * Remap structural ids using `idMap` (variant → new master). Non-subtree `ref`
 * values are kept (they should already point at master anchors). The subtree
 * root omits `ref` on the master row.
 */
function remapSubtreeBlockDataForMasterInsert(args: {
  data: (typeof blockDataValidator)["type"];
  idMap: Map<Id<"blocks">, Id<"blocks">>;
  isRoot: boolean;
}): (typeof blockDataValidator)["type"] {
  const mapId = (id: Id<"blocks">) => args.idMap.get(id) ?? id;
  const mapOptionalRef = (
    ref: Id<"blocks"> | undefined
  ): Id<"blocks"> | undefined => {
    if (args.isRoot) return undefined;
    if (ref === undefined) return undefined;
    return mapId(ref);
  };

  const d = cloneBlockData(args.data);

  if (d.type === "text") {
    const ref = mapOptionalRef(d.ref);
    return {
      type: "text",
      text: d.text,
      align: d.align,
  ...
  if (d.type === "section") {
    ...
    return {
      type: "section",
      children: d.children.map((c) => mapId(c)),
      ...(ref !== undefined ? { ref } : {}),
    };
  }
  if (d.type === "columns") {
...
async function computeMasterLayoutInsertIndexForNewVariantRoot(args: {
  ctx: MutationCtx;
  masterLayout: Id<"blocks">[];
  variantLayout: Id<"blocks">[];
  variantNewId: Id<"blocks">;
  variantBlocksById: Map<Id<"blocks">, Doc<"blocks">>;
  masterDocumentId: Id<"documents">;
}): Promise<number> {
  const idx = args.variantLayout.indexOf(args.variantNewId);
  if (idx === -1) return args.masterLayout.length;

  let prevMaster: Id<"blocks"> | null = null;
  for (let j = idx - 1; j >= 0; j--) {
    const row = args.variantBlocksById.get(args.variantLayout[j]!);
    if (!row) continue;
    const anchor = await masterAnchorRefForLayoutRow(
      args.ctx,
      row,
      args.masterDocumentId
    );
    if (anchor) {
      prevMaster = anchor;
      break;
  ...
  let nextMaster: Id<"blocks"> | null = null;
  for (let j = idx + 1; j < args.variantLayout.length; j++) {
    const row = args.variantBlocksById.get(args.variantLayout[j]!);
    if (!row) continue;
    const anchor = await masterAnchorRefForLayoutRow(
      args.ctx,
  ...
  if (nextMaster) {
    const i = args.masterLayout.indexOf(nextMaster);
    return i === -1 ? args.masterLayout.length : i;
  }
  if (prevMaster) {
    const i = args.masterLayout.indexOf(prevMaster);
    return i === -1 ? args.masterLayout.length : i + 1;
  }
  return args.masterLayout.length;
}

async function duplicateVariantRootSubtreeOntoMaster(args: {
  ctx: MutationCtx;
  masterDocumentId: Id<"documents">;
  variantRootId: Id<"blocks">;
  variantBlocksById: Map<Id<"blocks">, Doc<"blocks">>;
}): Promise<{
  masterRootId: Id<"blocks">;
  idMap: Map<Id<"blocks">, Id<"blocks">>;
}> {
  const idMap = new Map<Id<"blocks">, Id<"blocks">>();

  const ensureMasterId = async (
    variantId: Id<"blocks">,
    isRoot: boolean
  ): Promise<Id<"blocks">> => {
    const existing = idMap.get(variantId);
    if (existing) {
      return existing;
    }

    const row = args.variantBlocksById.get(variantId);
    if (!row) {
      throw new Error("Missing variant block while syncing subtree to master.");
    }

    if (!isRoot) {
      const reusableMasterId = await masterAnchorRefForLayoutRow(
        args.ctx,
        row,
        args.masterDocumentId
      );
      if (reusableMasterId) {
        idMap.set(variantId, reusableMasterId);
        return reusableMasterId;
      }
    }

    for (const childId of getStructuralChildBlockIds(row.data)) {
      await ensureMasterId(childId, false);
    }

    const data = remapSubtreeBlockDataForMasterInsert({
      data: row.data,
      idMap,
      isRoot,
    });
    const newId = await args.ctx.db.insert("blocks", {
      documentId: args.masterDocumentId,
      data,
    });
    idMap.set(variantId, newId);
    return newId;
  };

  const masterRootId = await ensureMasterId(args.variantRootId, true);
  return { masterRootId, idMap };
}

async function patchVariantBlockDataWithMasterRef(args: {
  ctx: MutationCtx;
  variantBlockId: Id<"blocks">;
  masterAnchorId: Id<"blocks">;
}): Promise<void> {
  const row = await args.ctx.db.get(args.variantBlockId);
  if (!row) {
    throw new Error("Variant block missing when linking to master anchor.");
  }
  const d = row.data;
  if (d.type === "text") {
    await args.ctx.db.patch(args.variantBlockId, {
      data: {
        type: "text",
        text: d.text,
        align: d.align,
...
async function patchVariantSubtreeDataWithMasterRefs(args: {
  ctx: MutationCtx;
  variantToMasterIdMap: Map<Id<"blocks">, Id<"blocks">>;
  variantBlocksById: Map<Id<"blocks">, Doc<"blocks">>;
}): Promise<void> {
  for (const [variantBlockId, masterAnchorId] of args.variantToMasterIdMap) {
    await patchVariantBlockDataWithMasterRef({
      ctx: args.ctx,
      variantBlockId,
      masterAnchorId,
    });
    const refreshed = await args.ctx.db.get(variantBlockId);
    if (refreshed) {
      args.variantBlocksById.set(variantBlockId, refreshed);
    }
  }
}

async function computeMasterChildInsertIndexForNewVariantChild(args: {
  ctx: MutationCtx;
  masterParentRow: Doc<"blocks">;
  variantParentRow: Doc<"blocks">;
  variantNewChildId: Id<"blocks">;
  variantBlocksById: Map<Id<"blocks">, Doc<"blocks">>;
  masterDocumentId: Id<"documents">;
}): Promise<number> {
  const variantChildren = getStructuralChildRefs(args.variantParentRow.data);
  const masterChildren = getStructuralChildRefs(args.masterParentRow.data);
  const idx = variantChildren.findIndex(
    (child) => child.childId === args.variantNewChildId
  );
  if (idx === -1) return masterChildren.length;

  let prevMaster: Id<"blocks"> | null = null;
  for (let j = idx - 1; j >= 0; j--) {
    const siblingRow = args.variantBlocksById.get(variantChildren[j]!.childId);
    ...
    if (
      anchor &&
      masterChildren.some((masterChild) => masterChild.childId === anchor)
    ) {
      prevMaster = anchor;
      break;
    }
  ...
  let nextMaster: Id<"blocks"> | null = null;
  for (let j = idx + 1; j < variantChildren.length; j++) {
    const siblingRow = args.variantBlocksById.get(variantChildren[j]!.childId);
    if (!siblingRow) continue;
    const anchor = await masterAnchorRefForLayoutRow(
      args.ctx,
  ...
  if (nextMaster) {
    const insertIdx = masterChildren.findIndex(
      (child) => child.childId === nextMaster
    );
    return insertIdx === -1 ? masterChildren.length : insertIdx;
  }
  if (prevMaster) {
    const insertIdx = masterChildren.findIndex(
      (child) => child.childId === prevMaster
    );
    return insertIdx === -1 ? masterChildren.length : insertIdx + 1;
  }
  return masterChildren.length;
}
...
export const updateDocumentBlocks = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const previousUpdatedBlocksById = new Map<Id<"blocks">, Doc<"blocks">>();
    let created = 0;
    let updated = 0;
    let deleted = 0;
    ...
    for (const action of updates) {
      ...
      if (!previousUpdatedBlocksById.has(mappedBlockId)) {
        previousUpdatedBlocksById.set(mappedBlockId, block);
      }
    ...
    const masterRow = await ctx.db
      ...
      .first();
    const masterDocumentId = masterRow?.documentId ?? null;

    if (
      masterDocumentId !== null &&
      args.documentId !== masterDocumentId &&
      createdBlockIds.length > 0
    ) {
      const masterDoc = await ctx.db.get(masterDocumentId);
      if (masterDoc) {
        const createdBlockIdSet = new Set(createdBlockIds);
        const variantAllBlocks = await ctx.db
          .query("blocks")
          .withIndex("by_documentId", (q) => q.eq("documentId", args.documentId))
          .collect();
        const variantBlocksById = new Map(
          variantAllBlocks.map((b) => [b._id, b])
        );

        let masterLayout = [...masterDoc.layout];

        if (mappedLayoutAfterMutation !== undefined) {
          const rootCreatedIdSet = new Set(
            createdBlockIds.filter((id) => mappedLayoutAfterMutation.includes(id))
          );
          const newRootIdsInLayoutOrder = mappedLayoutAfterMutation.filter((id) =>
            rootCreatedIdSet.has(id)
          );

          for (const variantNewId of newRootIdsInLayoutOrder) {
            const candidateRow = variantBlocksById.get(variantNewId);
            if (!candidateRow) continue;

            const existingRef = getOptionalDataRef(candidateRow.data);
            if (existingRef) {
              const refTarget = await ctx.db.get(existingRef);
              if (refTarget?.documentId === masterDocumentId) {
                continue;
              }
            }

            const insertIdx =
              await computeMasterLayoutInsertIndexForNewVariantRoot({
                ctx,
                masterLayout,
                variantLayout: mappedLayoutAfterMutation,
                variantNewId,
                variantBlocksById,
                masterDocumentId,
              });

            const { masterRootId, idMap } =
              await duplicateVariantRootSubtreeOntoMaster({
              ctx,
              masterDocumentId,
              variantRootId: variantNewId,
              variantBlocksById,
            });

            masterLayout = [
              ...masterLayout.slice(0, insertIdx),
              masterRootId,
              ...masterLayout.slice(insertIdx),
            ];
            await ctx.db.patch(masterDocumentId, { layout: masterLayout });

            await patchVariantSubtreeDataWithMasterRefs({
              ctx,
              variantToMasterIdMap: idMap,
              variantBlocksById,
            });
          }
        }

        for (const [variantParentId, oldParentRow] of previousUpdatedBlocksById) {
          if (createdBlockIdSet.has(variantParentId)) continue;
          if (oldParentRow.data.type === "text") continue;

          const variantParentRow = variantBlocksById.get(variantParentId);
          if (!variantParentRow || variantParentRow.data.type === "text") {
            continue;
          }

          const masterParentId = await masterAnchorRefForLayoutRow(
            ctx,
            variantParentRow,
            masterDocumentId
          );
          if (!masterParentId) continue;

          const masterParentRowInitial = await ctx.db.get(masterParentId);
          if (
            !masterParentRowInitial ||
            masterParentRowInitial.data.type === "text"
          ) {
            continue;
          }
          let masterParentRow: Doc<"blocks"> = masterParentRowInitial;

          const oldChildIds = new Set(
            getStructuralChildRefs(oldParentRow.data).map((child) => child.childId)
          );
          const insertedChildren = getStructuralChildRefs(
            variantParentRow.data
          ).filter(
            (child) =>
              createdBlockIdSet.has(child.childId) && !oldChildIds.has(child.childId)
          );

          for (const insertedChild of insertedChildren) {
            const candidateRow = variantBlocksById.get(insertedChild.childId);
            if (!candidateRow) continue;
            ...
            const insertIdx = await computeMasterChildInsertIndexForNewVariantChild(
              {
                ctx,
                masterParentRow,
                variantParentRow,
                variantNewChildId: insertedChild.childId,
                variantBlocksById,
                masterDocumentId,
              }
            );
            ...
            const { masterRootId, idMap } =
              await duplicateVariantRootSubtreeOntoMaster({
                ctx,
                masterDocumentId,
                variantRootId: insertedChild.childId,
                variantBlocksById,
              });

            await ctx.db.patch(masterParentRow._id, {
              data: insertStructuralChildIntoData({
                data: masterParentRow.data,
                insertIndex: insertIdx,
                childId: masterRootId,
                span: insertedChild.span,
              }),
            });
            ...
            const refreshedMasterParentRow: Doc<"blocks"> | null =
              await ctx.db.get(masterParentRow._id);
            if (
              refreshedMasterParentRow &&
              refreshedMasterParentRow.data.type !== "text"
            ) {
              masterParentRow = refreshedMasterParentRow;
            }
          }
        }
      }
