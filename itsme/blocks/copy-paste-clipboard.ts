...
export function serializeCopyPasteClipboard(blocks: Block[]): string {
  const cleaned = stripDanglingTextRefsInSubtree(blocks);
 ...
 */
export function parseCopyPasteClipboardPayload(text: string): Block[] | null {
  let raw: unknown;
  try {
  ...
  const cleaned = stripDanglingTextRefsInSubtree(parsed.data.blocks);
  return remapCopyPasteBlocksToClientIds(cleaned);
}
